<?php defined('BASEPATH') OR exit('No direct script access allowed');
    class Msitemap extends CI_Model{
        public function post($limit, $offset){
            $this->db->select('permalink, posted_at, perusahaan_name, url');
        ...
        public function kategori($limit, $offset){
            $this->db->select('category_name, url, count(loker.category_id) as total');
        ...
        public function perusahaan($limit, $offset){
            $this->db->select('perusahaan_name, url');
            $this->db->limit($limit, $offset);

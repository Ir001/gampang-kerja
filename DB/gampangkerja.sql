...
CREATE TABLE `admin` (
  `id` int(11) NOT NULL,
  `fullname` varchar(88) DEFAULT NULL,
...
INSERT INTO `iklan` (`id`, `nama`, `posisi`, `status`, `code`) VALUES
(1, 'Iklan #1', 'bottom', NULL, '//');
...
CREATE TABLE `industri` (
  `id` int(11) NOT NULL,
  `industri_name` varchar(88) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `kabupaten` (
  ...
  `jenis` int(3) NOT NULL COMMENT '1 = Kab, 2 =Kota'
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `kabupaten` (`id_kab`, `id_prov`, `nama`, `jenis`) VALUES
...
CREATE TABLE `perusahaan` (
  ...
  `waktu_kerja` varchar(88) DEFAULT NULL,
  `ukuran_perusahaan` varchar(88) DEFAULT NULL,
  `description` text,
  `why_join_us` text
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
ALTER TABLE `kabupaten`
  ...
  ADD KEY `FK_kabupaten_provinsi` (`id_prov`);
...
ALTER TABLE `loker`
  ADD PRIMARY KEY (`id`),
  ADD KEY `FK_loker_category` (`category_id`),
  ADD KEY `FK_loker_perusahaan` (`perusahaan_id`),
  ADD KEY `FK_loker_kabupaten` (`kab_id`),
  ADD KEY `FK_loker_provinsi` (`prov_id`);
...
ALTER TABLE `perusahaan`
  ADD PRIMARY KEY (`id`),
  ADD KEY `FK_perusahaan_industri` (`industri_id`);
...
--
ALTER TABLE `kabupaten`
  ADD CONSTRAINT `FK_kabupaten_provinsi` FOREIGN KEY (`id_prov`) REFERENCES `provinsi` (`id_prov`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `loker`
--
ALTER TABLE `loker`
  ADD CONSTRAINT `FK_loker_category` FOREIGN KEY (`category_id`) REFERENCES `category` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_loker_kabupaten` FOREIGN KEY (`kab_id`) REFERENCES `kabupaten` (`id_kab`) ON DELETE CASCADE ON UPDATE CASCADE,
  ...
  ADD CONSTRAINT `FK_loker_provinsi` FOREIGN KEY (`prov_id`) REFERENCES `provinsi` (`id_prov`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `perusahaan`
--
ALTER TABLE `perusahaan`
  ADD CONSTRAINT `FK_perusahaan_industri` FOREIGN KEY (`industri_id`) REFERENCES `industri` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
